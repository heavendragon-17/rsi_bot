...
def test_pnl_calculation():
    ...
    buy_signal = SignalEvent(
        symbol="DOGE/USDT", signal_type="BUY", price=entry_price, timestamp=datetime.now(),
        sl_price=sl_price, tp1_price=tp1_price, lock_profit_price=lock_price,
    ...
    tp1_signal = SignalEvent(
        symbol="DOGE/USDT", signal_type="SELL", price=tp1_price, 
        reason="TP1 hit", timestamp=datetime.now(), sl_price=lock_price # Move SL
    )
    ...
    for t in history:
        print(f"Type: {t['type']}, Side: {t['side']}, Amt: {t['amount']}, Price: {t['price']}, PnL: {t['pnl']}, Fee: {t.get('fee', {}).get('cost', 0)}")
        if t['pnl']: total_pnl += t['pnl']
        if t['fee']: total_fee += t['fee']['cost']
