...
def test_wick_below_sl_no_close(strategy, monkeypatch):
    ...
    assert not any(
        isinstance(a, ClosePosition) for a in result.actions
    ), "Wick through soft SL must not trigger ClosePosition"
    assert not result.new_context.meta.get("pending_candle_sl", False), "pending_candle_sl must stay False on a wick"
...
def test_close_below_sl_sets_flag(strategy, monkeypatch):
    ...
    assert isinstance(
        result.actions[0], DoNothing
    ), "First candle closing below soft SL returns DoNothing (2-candle pattern)"
    assert result.new_context.meta.get("pending_candle_sl"), "pending_candle_sl must be set to True"
...
def test_context_resets_to_scanning_after_close(strategy, monkeypatch):
    ...
    assert result.new_context.state == "SCANNING", "Context must reset to SCANNING after candle-close exit"
    assert not result.new_context.meta.get("pending_candle_sl", False), "pending_candle_sl must be cleared after exit"
