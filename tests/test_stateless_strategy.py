...
def test_analyze_returns_analysis_result(strategy, monkeypatch):
    ...
    assert isinstance(result, AnalysisResult), f"analyze() must return AnalysisResult, got {type(result)}"
...
def test_analyze_does_not_mutate_self_context(strategy, monkeypatch):
    ...
    assert strategy.context is original_context, "analyze() must not replace self.context"
...
def test_same_inputs_same_outputs(strategy, monkeypatch):
    ...
    assert type(r1.actions[0]) is type(r2.actions[0]), (  # noqa: E721
        "Same inputs must produce same action type"
    )
    assert r1.new_context.state == r2.new_context.state, "Same inputs must produce same new_context state"
...
def test_no_position_returns_do_nothing_or_scanning(strategy, monkeypatch):
    ...
    assert any(
        isinstance(a, DoNothing) for a in result.actions
