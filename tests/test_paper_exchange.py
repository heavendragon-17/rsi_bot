...
"""
Unit tests for PaperExchange order simulation engine.

Tests are isolated — each creates a fresh PaperExchange with a mocked notifier
...
def exchange(monkeypatch):
    """PaperExchange with the Telegram notifier fully mocked out."""
    mock_notifier = MagicMock()
    with patch("app.paper.exchange.PaperExchange.__init__", autospec=True) as mock_init:
        # We construct the object ourselves to inject mocked notifier cleanly
        pass

    # Build real exchange, but replace notifier after construction
    ex = _make_exchange()
    ex.notifier = MagicMock()
    return ex


def _make_exchange(balance: float = 10_000) -> PaperExchange:
    """Construct PaperExchange without touching Telegram."""
    cfg = _config(balance)
    with patch("app.paper.notifier.PaperTelegramNotifier.__init__", return_value=None):
...
def _open_position(ex: PaperExchange, symbol="BTC/USDT", amount="0.01", entry="95000"):
