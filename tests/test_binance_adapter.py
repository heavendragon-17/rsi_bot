...
from app.trading.exchange.binance_adapter import BinanceAdapter  # noqa: E402
...
class TestBinancePaperTrading:
    ...
    def adapter(self):
        ...
        return BinanceAdapter(
            config={"bot": {"mode": "paper"}}, initial_balance=10000.0, leverage=10, maker_fee=0.0002, taker_fee=0.0004
        )
    ...
    def test_simulated_order_with_real_prices(self, adapter):
        ...
        order2 = adapter.create_order(symbol=symbol, order_type="market", side="sell", amount=Decimal("0.01"))
    ...
    def test_paper_trading_workflow(self, adapter):
        ...
        buy_order = adapter.create_order(symbol=symbol, order_type="market", side="buy", amount=Decimal("0.01"))
        ...
        # 7. Close position
        print("\nClosing position...")
        sell_order = adapter.create_order(symbol=symbol, order_type="market", side="sell", amount=Decimal("0.01"))
    ...
    def test_multiple_positions(self, adapter):
        ...
        for symbol in symbols:
            ...
            adapter.create_order(symbol=symbol, order_type="market", side="buy", amount=Decimal("0.01"))
...
class TestLiveModeCaution:
    ...
    def test_api_key_setup(self, adapter):
        ...
        try:
            ...
            print("  API keys are valid for market data")
...
if __name__ == "__main__":
    ...
    adapter = BinanceAdapter(config={"bot": {"mode": "paper"}}, initial_balance=10000.0, leverage=10)
    ...
    try:
        ...
        print("\n" + "=" * 60)
        adapter2 = BinanceAdapter(config={"bot": {"mode": "paper"}}, initial_balance=10000.0, leverage=10)
        test_suite.test_simulated_order_with_real_prices(adapter2)

        print("\n" + "=" * 60)
        adapter3 = BinanceAdapter(config={"bot": {"mode": "paper"}}, initial_balance=10000.0, leverage=10)
        test_suite.test_simulated_limit_order(adapter3)

        print("\n" + "=" * 60)
        adapter4 = BinanceAdapter(config={"bot": {"mode": "paper"}}, initial_balance=10000.0, leverage=10)
...
class TestBinanceTestnetIntegration:
    ...
    def test_simulated_limit_order(self, adapter):
        ...
        order = adapter.create_order(
            symbol=symbol, order_type="limit", side="buy", amount=Decimal("0.001"), price=limit_price
        )
    ...
    def test_stop_loss_simulation(self, adapter):
        ...
        adapter.create_order(symbol=symbol, order_type="market", side="sell", amount=Decimal("0.01"))
    ...
    def test_market_order_workflow(self, adapter):
        ...
        try:
            ...
            buy_order = adapter.create_order(symbol=symbol, order_type="market", side="buy", amount=position_size)
            ...
            if positions:
                ...
                sl_order = adapter.place_stop_loss(
                    symbol=symbol, side="sell", amount=position_size, stop_price=stop_price
                )
                if sl_order:
            ...
            sell_order = adapter.create_order(symbol=symbol, order_type="market", side="sell", amount=position_size)
    ...
    def test_fetch_ohlcv(self, adapter):
        ...
        if candles:
            latest = candles[-1]
            print(
                f"Latest candle: timestamp={latest[0]}, "
