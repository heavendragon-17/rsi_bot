...
class TestMockExchangeThreadSafety:
    ...
    def test_concurrent_order_placement(self):
            ...
            for _ in range(iterations):
                try:
                    ...
                    order = exchange.create_order(
                        symbol=symbol, order_type="market", side="BUY", amount=amount, price=price
                    )
                    if order:
                        with lock:
                    ...
                    if pos > 0:
                        ...
                        sell_order = exchange.create_order(
                            symbol=symbol, order_type="market", side="SELL", amount=sell_amt, price=price
                        )
                        if sell_order:
                            with lock:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(worker, sym, 20) for sym in symbols]
