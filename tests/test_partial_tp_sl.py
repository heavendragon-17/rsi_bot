...
def test_partial_tp_and_sl_move(setup):
    config, exchange, portfolio, strategy = setup
    symbol = "BTC/USDT"
    ...
    # Mock Exchange State
    exchange.positions[symbol] = initial_amount
    exchange.orders = {
        "mock_sl_id": {
            "id": "mock_sl_id",
            "symbol": symbol,
    ...
    assert move_sl_action is not None, "Should have generated a MoveSL action for lock profit"
    assert "MOVE_SL_LOCK_PROFIT" in move_sl_action.reason
    ...
    assert move_sl_action.new_sl_price is not None
    assert float(move_sl_action.new_sl_price) == pytest.approx(float(expected_sl), abs=1e-2)
    ...
    exchange.cancel_order = lambda oid, sym: True
    exchange.create_order = mock_create_order

    # Run portfolio logic using new execute_partial_close path
    portfolio.execute_partial_close(symbol, "TP1", new_sl_price=move_sl_action.new_sl_price)
    ...
    assert float(last_sl_order["params"]["stopPrice"]) == pytest.approx(float(expected_sl), abs=1e-2)
    assert last_sl_order["amount"] == Decimal("1.0")
    assert last_sl_order["params"].get("reduceOnly")
