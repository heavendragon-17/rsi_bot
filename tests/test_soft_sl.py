...
def setup():
    ...
    strategy.context.trades = {}  # Reset trades
    return config, exchange, portfolio, strategy


def test_soft_sl_activation(setup):
    ...
    exchange.positions[symbol] = Decimal("1")
    exchange.margin_used[symbol] = Decimal("100")
    exchange.entry_prices[symbol] = entry_price
    ...
    # 3. Run Analyze
    signal = strategy.analyze(symbol, df_mock)

    # 4. Verify Signal
    assert signal is not None, "Should have generated a signal"
    assert signal.signal_type == "SELL"
    assert signal.reason == "CLOSE_BY_CANDLE_SL"

    # Check Price (should have slippage)
    # 94 * (1 - 0.001) = 94 * 0.999 = 93.906
    expected_price = Decimal("94") * Decimal("0.999")
    assert float(signal.price) == pytest.approx(float(expected_price), abs=1e-4)
