...
import { Check, AlertTriangle, X, TrendingUp, Target, Award } from "lucide-react";
...
export const ResultsSummary: React.FC = () => {
  ...
  const paramLabel = AVAILABLE_PARAMETERS.find(p => p.value === paramToOptimize)?.label || paramToOptimize;
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold text-text-primary">Walk-Forward Results</h3>

      {/* Main metrics cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-4 bg-bg-elevated border-border-main">
          <div className="flex items-start justify-between mb-2">
            <div className="text-xs font-medium text-text-secondary">OOS WIN RATE</div>
        ...
        <Card className="p-4 bg-bg-elevated border-border-main">
          <div className="flex items-start justify-between mb-2">
            <div className="text-xs font-medium text-text-secondary">AVG OOS RETURN</div>
          ...
          <div className="space-y-1">
            <div className={cn(
              ...
              {summary.avgOosReturn > 0 ? "+" : ""}{summary.avgOosReturn.toFixed(2)}%
            </div>
            <div className="text-xs text-text-secondary">
              per window
            </div>
        ...
        <Card className={cn(
          "p-4 border-2",
          summary.verdict === "robust" ? "bg-success/5 border-success/30" :
          summary.verdict === "marginal" ? "bg-warning/5 border-warning/30" :
          "bg-danger/5 border-danger/30"
        )}>
          <div className="flex items-start justify-between mb-2">
            <div className="text-xs font-medium text-text-secondary">ROBUSTNESS</div>
            {getVerdictIcon()}
          </div>
          <div className="space-y-1">
            <div className={cn(
      ...
      <Card className="p-4 bg-bg-elevated border-border-main">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <div className="text-text-secondary text-xs mb-1">Total OOS Return</div>
            <div className={cn(
              "font-semibold",
              summary.totalOosReturn > 0 ? "text-success" : "text-danger"
            )}>
              {summary.totalOosReturn > 0 ? "+" : ""}{summary.totalOosReturn.toFixed(2)}%
            </div>
          </div>
          ...
          <div>
            ...
            <div className="font-semibold text-text-primary">
              W{summary.bestWindow.index} ({summary.bestWindow.returnPct > 0 ? "+" : ""}
          ...
          <div>
            ...
            <div className="font-semibold text-text-primary">
              W{summary.worstWindow.index} ({summary.worstWindow.returnPct > 0 ? "+" : ""}
              {summary.worstWindow.returnPct.toFixed(1)}%)
            </div>
          </div>

          <div>
            <div className="text-text-secondary text-xs mb-1">Parameter Stability</div>
            <div className={cn("font-semibold uppercase text-xs", getStabilityColor())}>
      ...
      <Card className="p-4 bg-accent-main/5 border border-accent-main/30">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs text-text-secondary mb-1">Most Common Best Parameter</div>
            <div className="text-lg font-bold text-text-primary">
              {paramLabel} = {summary.mostCommonParam.value}
            </div>
            <div className="text-xs text-text-secondary mt-1">
              Appeared in {summary.mostCommonParam.count} / {summary.totalWindows} windows 
              ({((summary.mostCommonParam.count / summary.totalWindows) * 100).toFixed(0)}%)
            </div>
          </div>
          <Button
