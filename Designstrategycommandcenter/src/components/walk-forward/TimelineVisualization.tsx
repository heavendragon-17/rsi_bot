  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-text-primary">Walk-Forward Timeline</h3>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3">
        {windows.map((window) => (
          <WindowBlock key={window.index} window={window} paramName={paramLabel} isCompact />
        ))}
      </div>
      ...
      <div className="relative pt-8">
        ...
        <div className="flex justify-between mt-2 text-xs text-text-tertiary">
          ...
          <span>{windows[windows.length - 1]?.oosEndDate || "Dec 31, 2024"}</span>
        </div>
      </div>
    </div>
  );
