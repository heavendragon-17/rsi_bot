...
export const PortfolioEquityChart: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Or better: Use "Extra Series" logic.
    const maxSeries = chart.addSeries(LightweightCharts.LineSeries, {
    ...
    const highSeries = chart.addSeries(LightweightCharts.LineSeries, { color: `${dispersionColor}44`, lineWidth: 1, lineStyle: LineStyle.Dotted, crosshairMarkerVisible: false, lastValueVisible: false, priceLineVisible: false });
    highSeries.setData(dispHigh);
    const lowSeries = chart.addSeries(LightweightCharts.LineSeries, { color: `${dispersionColor}44`, lineWidth: 1, lineStyle: LineStyle.Dotted, crosshairMarkerVisible: false, lastValueVisible: false, priceLineVisible: false });
    ...
    // 3. Benchmark (Dashed)
    const benchmarkSeries = chart.addSeries(LightweightCharts.LineSeries, {
    ...
    pinnedSymbols.forEach((sym, idx) => {
        ...
        if (symData) {
            ...
            const pinSeries = chart.addSeries(LightweightCharts.LineSeries, {
