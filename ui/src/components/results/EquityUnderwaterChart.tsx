...
export const EquityUnderwaterChart: React.FC = () => {
  ...
  const { equityCurve, benchmarkCurve, underwaterCurve, netProfit } = useResultsStore();
  ...
  useEffect(() => {
    ...
    // Check if addAreaSeries exists before calling
    let strategySeries;
    if (typeof chart.addAreaSeries === 'function') {
        strategySeries = chart.addAreaSeries({
        ...
        console.error("LightweightCharts: addAreaSeries not found on chart instance");
    }
  ...
  useEffect(() => {
      ...
      const handleResize = () => {
        if (underwaterContainerRef.current) {
            chart.applyOptions({ width: underwaterContainerRef.current.clientWidth });
        }
      };
      window.addEventListener("resize", handleResize);
