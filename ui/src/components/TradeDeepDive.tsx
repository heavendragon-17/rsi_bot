...
export function TradeDeepDive({ trade, onClose }: TradeDeepDiveProps) {
  ...
  useEffect(() => {
    ...
    async function fetchChart() {
      ...
      setError(null);
      try {
        const data = await getTradeChart(trade.id.toString());
        if (!mounted) return;

        const formatted = data.map((c, i) => {
          const dateObj = new Date(c.time);
          const isEntry =
            dateObj.getTime() === new Date(trade.entryTime).getTime();
          ...
          return {
            ...
            low: c.low,
            close: c.close,
            price: c.close, // Used by the Area chart
            volume: c.volume || 0,
            rsi: c.rsi,
            wma45: c.wma45,
            ema9: c.ema9,
            active_sl: c.active_sl,
            lock_profit_active: c.lock_profit_active,
            isEntry: c.is_entry || isEntry,
            isExit: c.is_exit || false,
          };
        });

        // Ensure we mark the last valid candle as exit if not provided by backend
        if (!formatted.find((f) => f.isExit) && formatted.length > 0) {
          formatted[formatted.length - 1].isExit = true;
        }
            ...
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm rounded-xl">
              <div className="flex flex-col items-center gap-3">
                ...
                <span className="text-slate-300 font-medium">
                  Loading high-resolution tick data...
                </span>
              </div>
            </div>
          )}
          ...
          <div className="bg-slate-900/50 rounded-xl p-6 border border-white/10">
            ...
            <ResponsiveContainer width="100%" height={150}>
              <ComposedChart data={chartData}>
                ...
                />
                <Bar dataKey="volume" fill="#475569" radius={[4, 4, 0, 0]} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
