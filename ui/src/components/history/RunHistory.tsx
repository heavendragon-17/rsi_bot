...
export const RunHistory: React.FC = () => {
  const { runs, clearAllHistory, addRun } = useHistoryStore();

  const handleAddMockData = () => {
    const mockRuns = generateMockHistory(20);
    mockRuns.forEach(run => addRun(run));
  };
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border-main/50">
        ...
        <div className="flex gap-2">
          {runs.length === 0 && (
            <Button variant="outline" size="sm" className="gap-2" onClick={handleAddMockData}>
              <Plus size={14} />
              Add Sample Data
            </Button>
          )}
