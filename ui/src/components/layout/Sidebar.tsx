...
export const Sidebar: React.FC = () => {
  ...
  const executeRun = async () => {
    ...
    if (mode === "single") {
      ...
      // Get results from store after generation
      ...
      addRun({
        ...
        tradeCount: batchState.symbolResults.reduce(
          (sum, s) => sum + s.tradeCount,
          0
        ),
      });
    }
  ...
  const handleRunRequest = async () => {
    ...
    const symbolsToCheck = mode === "batch" ? batchSymbols : [symbol];
        ...
        >
          <div className="pb-2">
            ...
            <CollapsibleSection title="Mode" defaultOpen={true}>
              <div className="relative flex p-1 bg-bg-elevated/50 border border-border-main/50 rounded-lg overflow-hidden">
                {/* Sliding Background */}
                <motion.div
                  className="absolute inset-y-1 bg-accent-main rounded-md shadow-md z-0"
                  initial={false}
                  animate={{
                    x:
                      mode === "batch"
                ...
                <button
                  ...
                  className={cn(
                    "relative flex-1 py-1.5 text-[10px] font-bold rounded-md z-10 transition-colors duration-300 uppercase tracking-tight",
                    mode !== "batch" && mode !== "pine"
                      ? "text-[var(--bg-primary)]"
                ...
                <button
                  ...
                  className={cn(
                    "relative flex-1 py-1.5 text-[10px] font-bold rounded-md z-10 transition-colors duration-300 uppercase tracking-tight flex items-center justify-center gap-1",
                    mode === "pine"
                      ? "text-[var(--bg-primary)]"
                      : "text-text-secondary hover:text-text-primary"
                  )}
                ...
                <CollapsibleSection title="Asset Config">
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    {mode !== "batch" ? (
                      ...
                      <div className="col-span-2">
                        <label className="text-xs font-medium text-text-secondary mb-1.5 block">
                          Portfolio Assets (comma separated)
                        </label>
                        <textarea
                          defaultValue={batchSymbols.join(", ")}
                          onBlur={(e) =>
                            setBatchSymbols(
                              e.target.value
                                .split(",")
                          ...
                          className="w-full h-24 bg-input/50 border border-border-main rounded-md px-3 py-2 text-xs text-text-primary focus:outline-none focus:ring-1 focus:ring-accent-main/50 resize-none font-mono"
                ...
                >
                  <div className="space-y-3">
                    <ValidatedInput
                      ...
                      onChangeValue={(v) =>
                        setParam("rsi_period", parseInt(v, 10) || 0)
                      }
                    />
                    <ValidatedInput
                      label="EMA Fast"
                      ...
                      onChangeValue={(v) =>
                        setParam("ema_fast", parseInt(v, 10) || 0)
                      }
                    />
                    <ValidatedInput
                      label="EMA Slow"
                      ...
                      onChangeValue={(v) =>
                        setParam("ema_slow", parseInt(v, 10) || 0)
                      }
                    />
                    <ValidatedInput
                      ...
                      onChangeValue={(v) =>
                        setParam("tp1_rr", parseFloat(v) || 0)
                      }
                      suffix="R"
                    />
                    <ValidatedInput
                      ...
                      onChangeValue={(v) =>
                        setParam("sl_buffer_pct", parseFloat(v) || 0)
                      }
                      suffix="%"
                    />
                  </div>
