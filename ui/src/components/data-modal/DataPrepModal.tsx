...
import { startDownload, streamDownload } from "../../api/data";
...
export const DataPrepModal: React.FC = () => {
  ...
  // Real download: iterate pending symbols sequentially via API
  useEffect(() => {
    if (state !== "downloading" || !isOpen) return;
    ...
    const runDownloads = async () => {
      for (let i = 0; i < pendingSymbols.length; i++) {
        ...
        updateSymbolStatus(sym.symbol, { status: "downloading" });
        useDataPrepStore.setState({ currentDownload: sym.symbol });

        try {
          const { job_id } = await startDownload(sym.symbol, timeframe);
          await new Promise<void>((resolve, reject) => {
            const cleanup = streamDownload(
              job_id,
              (pct) => {
                const baseProgress = (i / pendingSymbols.length) * 100;
                setProgress(baseProgress + pct / pendingSymbols.length, 0);
              },
              () => { cleanup(); resolve(); },
          ...
          if (!cancelled) updateSymbolStatus(sym.symbol, { status: "fresh" });
        } catch {
          if (!cancelled) {
            updateSymbolStatus(sym.symbol, { status: "error" });
            setPrepState("error");
            return;
          }
        }
