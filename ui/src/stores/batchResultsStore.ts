...
export interface BatchResultsState {
    ...
    portfolioNetProfitPct: number;
    portfolioWinRate: number;
    portfolioProfitFactor: number;
    portfolioSharpe: number;
    portfolioMaxDrawdown: number; // This was missing, causing error

    totalPnL: number;
    totalPnLPct: number;
    benchmarkPnLPct: number;
    portfolioMaxDrawdownPct: number; // Keep existing specific one too if needed, but standardize naming
...
export const useBatchResultsStore = create<BatchResultsState>()(
    persist(
        (set, get) => ({
            ...
            portfolioNetProfitPct: 0,
            portfolioWinRate: 0,
            portfolioProfitFactor: 0,
            portfolioSharpe: 0,
            ...
            generateMockBatchResults: (capital, symbolsList) => {
                ...
                set({
                    ...
                    portfolioNetProfit: finalTotalPnL,
                    portfolioNetProfitPct: (finalTotalPnL / capital) * 100,
                    portfolioWinRate: 65, // Mock
                    portfolioProfitFactor: 1.5, // Mock
                    portfolioSharpe: 0.8 + Math.random() * 1.5,
                    portfolioMaxDrawdown: 5 + Math.random() * 15,
