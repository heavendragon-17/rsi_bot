...
>     `app/api/routes/backtest_stream.py`, `app/backtest/executor.py`
...
1. **POST endpoint**: Validate request → create DB parent row → submit job to executor (`app/backtest/executor.py` → `submit_backtest(job_id, fn)`) → return `{"run_id": ..., "status": "running"}`
