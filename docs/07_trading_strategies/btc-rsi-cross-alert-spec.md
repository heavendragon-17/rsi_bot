...
5. Alert only when the selected fully closed H1 and H4 candles close strictly
   above their native price EMA21 values.
6. Do not calculate or evaluate EMA9(RSI21) or WMA45(RSI21) for the H1/H4
   price gates.
7. Send the alert to Telegram.
8. For M5 only, also require an RSI smoothing-line spread of at least 2,
...
`IStrategy.analyze()` contract. That contract supplies one DataFrame, while
   this feature requires point-in-time access to M5, M15, H1, and H4 simultaneously.
...
- Subscribe to native Binance USD-M BTC candles for `5m`, `15m`, `1h`, and `4h`.
...
- Gate every M5/M15 signal using the latest fully closed native H1 and H4
  candles available at the trigger candle's close time.
...
- Fail closed when indicator, timing, continuity, or H1/H4 context is invalid.
...
native BTC/USDT `5m`, `15m`, `1h`, and `4h` OHLCV CSV files, reuses the locked
...
| Confirmation timeframe | Native Binance `1h` |
| Trend timeframe | Native Binance `4h` |
| Trigger policy | Closed candles only |

Do not resample M5 or M15 into H1/H4. Subscribe to Binance's native H1 and H4
streams and load native H1/H4 REST history.
...
- only H1 rows with close time `<= T` may be used;
- the selected H1 row must have the latest expected native UTC one-hour close
  at or before `T`;
...
If either exact expected H1 or H4 row has not arrived yet at a shared timeframe
...
Both trigger timeframes use the same RSI bundle:
...
H1 and H4 preparation compute EMA(21) directly over their native close-price
series and store the selected close, EMA21, and close time. They do not need an
RSI bundle.
...
- H4 needs at least 21 contiguous closed candles and a finite current price
  EMA21.
- H1 needs at least 21 contiguous closed candles and a finite current price
  EMA21.
...
4. For trigger data, compute RSI21, EMA9, and WMA45 over the entire contiguous
   suffix, not only its last 67 rows. For H1/H4, compute price EMA21 over the
   entire contiguous context suffix.
5. Require at least 67 rows in the selected trigger suffix or 21 rows in each
   selected H1/H4 suffix.
...
Evaluate only when a new closed candle arrives on `5m` or `15m`. H1/H4 closes
update context but never directly emit an alert.
...
Given the latest eligible fully closed H1 candle and its price EMA21:

```text
h1_bullish =
    h1_close_price > h1_price_ema21
```
...
M5 emits an alert if and only if `m5_bullish_alignment`, `h1_bullish`,
`h4_bullish`, and all four additional conditions below are true:
...
WMA45-level, RSI ceiling, and price EMA21 comparisons remain strict; equality
with WMA45 value `45`, RSI21 value `60`, or price EMA21 fails. The spread,
WMA45-level, and RSI ceiling conditions do not apply to M15; the trigger
close/EMA21(price) condition applies independently to both M5 and M15.
...
4. Only after H4 passes, evaluate H1. If H1 close is not strictly above H1
   EMA21(price), return `H1_CLOSE_NOT_ABOVE_EMA21`.
5. Only after the cross and H1/H4 gates pass, require M15 close > M15
   EMA21(price); otherwise return `M15_CLOSE_NOT_ABOVE_EMA21`.
6. Otherwise return `ALERT_FRESH_BULLISH_CROSS_H4_BULLISH`.
...
3. Require H1 close > H1 EMA21(price); otherwise return
   `H1_CLOSE_NOT_ABOVE_EMA21`.
4. Evaluate RSI21 < 60, spread, WMA45 level, then close-versus-price-EMA21
   in that order. Return the matching M5 rejection reason on the first failure.
5. Otherwise return `ALERT_M5_BULLISH_ALIGNMENT_H4_BULLISH`.
...
- The H1 close/EMA21(price) comparison is strict; equality fails the H1 gate.
...
- M5 also requires RSI21 < 60; equality at 60 fails.
...
the trigger, H1, and H4 DataFrames plus keyword-only `symbol`,
`trigger_timeframe`, `trigger_open_time`, `history_ready_at`, and an immutable
sets of `observed_live_h1_closes` and `observed_live_h4_closes`. The decision
function is named
...
indicator preparation, exact current-trigger selection, and exact H1/H4 context
selection. `evaluate_btc_rsi_cross()` owns the M15 fresh-cross and shared H1/H4
price-gate decision. `evaluate_m15_cross()` adds the M15 price-EMA21 filter.
`evaluate_m5_cross()` owns the M5 alignment, H1/H4, and M5-only filter decision.
...
- `m5_checker.py` fixes `trigger_timeframe="5m"` for preparation, accepts only
  prepared M5 inputs, and enforces the four mandatory M5-only filters;
...
indicator preparation to `evaluator.py`. M15 delegates fresh-cross/H1/H4
decision math to the shared evaluator and then applies its price-EMA21 filter;
M5 deliberately replaces fresh-cross with its own current-candle alignment and
additional filters.
...
the live H1/H4 confirmation sets, then passes immutable values into preparation.
...
- `H1_DUPLICATE_OR_NON_INCREASING_TIME`; and
- `H1_NON_FINITE_DATA`.
...
- `M5_CLOSE_NOT_ABOVE_EMA21`;
- `M15_CLOSE_NOT_ABOVE_EMA21`; and
- `M5_RSI21_NOT_BELOW_60`.
...
1. Accept closed `BTC/USDT` callbacks for `5m`, `15m`, `1h`, and `4h`.
   M5/M15 callbacks are evaluation triggers; H1/H4 callbacks only confirm that
   native context candles have closed and never emit an alert by themselves.
...
3. At history completion, record an aware UTC history-ready time and a
   ...
   be ignored.
4. Historical H1/H4 rows closing at or before the history-ready instant may be
   trusted as closed; a later context row is eligible only after its matching
   closed H1/H4 WebSocket callback has been observed.
5. Read trigger, H1, and H4 DataFrames from `TimeframeMultiplexer` using
   defensive copies supplied by `get_dataframe()`.
...
H1/H4 boundary coordination must not depend on a context item being processed
by the same queue that is waiting for it. The multiplexer callback handles live
closed H1/H4 events synchronously: under a thread-safe `Condition`, add the
aware UTC close to the matching observed-context set and notify waiters; do not
enqueue context candles for normal worker processing. When a trigger needs an
unconfirmed H1/H4 close, the worker waits on that condition for at most
`context_settle_seconds`, which releases the condition lock. It then
prepares/evaluates exactly once more.
...
- The resolver creates a typed BTC alert config and includes its four targets
...
- `_build_signal_startup_message()` must identify this component as
  `BTC/USDT · 5m,15m · H1/H4 filter`; it must not display the global symbol count
...
M5 RSI21 < 60.00: 53.42 < 60.00 ✅

H1 close: 65,000.00
H1 EMA21(price): 64,000.00
H1 close > EMA21(price): 65,000.00 > 64,000.00 ✅
...
or neutral H1/H4 gate, bootstrap history, duplicate callbacks, or internal retry.
...
| `docs/05_data_pipeline/live-data-flow.md` | Document four-stream data flow and bootstrap gate |
...
- already-aware timestamps represent the same instant without a second
  timezone shift;
- current trigger and expected H1/H4 rows are selected exactly as of `T`;
- future/forming H1/H4 candles are never used;
- post-bootstrap H1/H4 closes require membership in the matching observed-live set;
- 67 trigger rows and 21 H1/H4 rows are the exact readiness boundaries;
...
- exact bullish cross alerts when H1 and H4 closes are strictly above their
  native price EMA21 values;
...
- strict H1/H4 close > EMA21(price) passes;
- equality between either context close and EMA21(price) fails;
- H1/H4 close at or below EMA21(price) suppresses an otherwise valid cross;
...
- M5 rejects RSI21 at or above 60, spread below 2, WMA45 equal to or below 45,
  and BTC close equal to or below EMA21(price);
...
- open candles never trigger alerts, and H1/H4 callbacks only mark context closed;
- a post-bootstrap H1/H4 row cannot be used until its live closed callback is
  observed;
- exact H1/H4 contexts are selected as of the trigger close;
- boundary race retries once and succeeds when the missing context arrives;
- H1/H4 confirmations are recorded synchronously and wake a waiting trigger
  without requiring context events to pass through the worker queue;
...
- `tests/test_main_signal_mode.py`: startup text reports BTC plus M5/M15/H1/H4
...
- When the component is active, native BTC M5, M15, H1, and H4 streams are present
...
- A qualifying M5 bullish alignment or fresh M15 bullish crossover enqueues
  exactly one Telegram message when the exact eligible H1 and H4 contexts have
  close > EMA21(price), provided M5 also passes its four extra filters.
- The same crossover enqueues no message when either context close is less than
  or equal to EMA21(price), or when H1/H4 context is missing, forming, stale,
  unconfirmed at a live boundary, or not price-EMA-ready.
- An M5 alignment enqueues no message when RSI21 is >= 60, its RSI spread is
  < 2, its RSI WMA45 is <= 45, or its BTC close is <= EMA21(price).
...
- After a qualifying M5 alert, qualifying closes at +5m and +10m enqueue no
  message; a qualifying close at +60m may enqueue the next M5 message.
...
3. Verify the target-union log contains BTC M5, M15, H1, and H4.
...
- Only Binance BTC/USDT and the fixed M5/M15/H1/H4 rules are supported.
