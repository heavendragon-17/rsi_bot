...
* The startup banner's `Active components` count is one per active config
  component, including the standalone Core V2.1 runtime, not one per
  timeframe or Telegram route. A multi-timeframe component such as
  `btc_rsi_cross_alert` remains one component and lists its M5/M15 routes
  inline. The banner also identifies the runtime group, execution boundary,
  and Core Long V2's M15/25-candidate/LONG signal-only scope. Ordinary
  strategy labels may include a human-readable name followed by the
