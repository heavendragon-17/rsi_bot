...
| 19 | Deploy log repeats `Positions clear. Starting deploy` every minute but the service never restarts | `/var/log/rsi-bot-deploy.log` stops after the start message; `systemctl status rsi-bot` and `VERSION` remain on the previous release | `check_deploy.sh` reached an embedded Python runtime error after resetting the production checkout but before invoking `deploy.sh`; `bash -n` validates shell syntax, not Python names or imports | Inspect `sudo journalctl -u check-deploy.service -n 50` for the traceback. Run `tests/test_deploy_scripts.py`; the deploy helper now logs state-write errors and abnormal exits. After promoting a fixed tag, allow the timer to self-heal or use the guarded `deploy/force_deploy.sh <tag>` path |
