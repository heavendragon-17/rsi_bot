...
The bundle is committed to Git — developers refresh it with `release_ui.bat`
after UI changes — so a plain pull contains it and end users only need Python;
Node is not required while the application runs.
