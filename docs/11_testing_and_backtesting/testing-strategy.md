...
| `tests/test_signal_replay.py` | Historical CSV validation, UTC+7 normalization, full-card Markdown output, indicator warmup and initial warmup skipping, point-in-time future-row isolation, cached-preparation parity with existing checkers, M5 cooldown, duplicate suppression, manual-review fields, and repeatable rendering |
