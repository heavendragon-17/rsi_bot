# app/trading/strategy/rsi_no_retest_fade/strategy.py
"""
Layer 2: Core Logic - RSI No Retest FADE Strategy
=================================================

Fade variant of ``rsi_no_retest``: fires on the EXACT same bars as the
LONG parent (prolonged decline + EMA21 reclaim + bullish RSI spread)
but emits SELL instead of BUY. Tests the hypothesis that fading the
parent's entries preserves more signal than the mirror-trigger
``rsi_no_retest_short`` approach.

The trigger detection helpers (``detect_reclaim``, ``pullback_filter``)
are imported from the parent package — copying them would silently
diverge from the parent on future tweaks, which would defeat the
"same trigger, different bet" experiment.

Rules (summary):
- Entry: first candle closing > EMA21 (cross up) after prolonged decline
  (parent's trigger — IDENTICAL bars).
- RSI confirmation: bullish (RSI_EMA9 - RSI_WMA45) >= rsi_spread_min.
- Direction: SELL.
- SL: configurable mode (highest_close / highest_wick); sits ABOVE entry.
- TP: 1-3 levels at configurable RR ratios, BELOW entry.
- Exit: candle-close SL above, lock-profit trigger, pending candle SL,
  max-holding-period force-close (same as rsi_no_retest_short/exit.py).
"""
...
from app.trading.strategy.rsi_no_retest_fade.entry import check_entry
from app.trading.strategy.rsi_no_retest_fade.exit import manage_exit
...
class RsiNoRetestFadeConfig(SchemaConfigMixin):
    """Typed config for RsiNoRetestFadeStrategy.

    Parameter set is identical to the LONG parent (the trigger is the
    parent's trigger) except ``nr_sl_mode`` enum lists SHORT-flavored
    modes since the position is a SHORT.
    """

    METADATA = RSI_NO_RETEST_FADE_METADATA
    UI_GROUPS = RSI_NO_RETEST_FADE_GROUPS
...
class RsiNoRetestFadeStrategy(BaseStrategy):
    """RSI No Retest FADE — fires on the parent's trigger, bets the other way."""

    CONFIG_CLASS = RsiNoRetestFadeConfig
    ...
    def __init__(self, config: dict):
        ...
        self.strategy_cfg = merge_config(RsiNoRetestFadeConfig, cfg)
        ...
        # Parent's trigger uses `nr_max_above_ema21` — name preserved.
        self.max_above_ema21 = int(cfg.get("nr_max_above_ema21", 3))
    ...
    def analyze(
        ...
        # SHORT exit-management needs `low` (lock-profit trigger sits below entry).
