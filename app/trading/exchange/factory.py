...
def _load_custom_adapter(exchange_name: str, config: dict[str, Any]) -> IExchange:
    ...
    try:
        ...
        raise ValueError(
            f"Could not load adapter for '{exchange_name}'. " f"Module '{module_name}' not found or import failed: {e}"
        ) from e
    except AttributeError as e:
        raise ValueError(f"Module '{module_name}' found, but class '{class_name}' not defined.") from e
