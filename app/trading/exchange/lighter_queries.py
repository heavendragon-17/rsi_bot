...
def _find_sub_account(raw: dict, account_index: int) -> Optional[dict]:
...
def fetch_balance(adapter: LighterAdapter, params: Dict = {}) -> Dict:
    """
    ...
    async def _fetch():
        ...
        try:
            ...
            if not account_data:
                logger.warning(
                    f"Account index {adapter.account_index} not found in response"
                )
                return {
                    'info': raw,
                    'free': {'USDT': 0.0},
            ...
            try:
                free_balance = (
                    float(available_str) if available_str else total_balance
                )
            except (ValueError, TypeError):
    ...
    async def _fetch():
        ...
        try:
            ...
            if hasattr(order_api, 'get_order'):
                ...
                logger.warning(
                    "fetch_order: get_order method not available in SDK"
                )
    ...
    async def _fetch():
        ...
        try:
            ...
            if not hasattr(order_api, 'get_active_orders'):
                logger.warning(
                    "fetch_open_orders: get_active_orders not available"
                )
                return []

            resp = await order_api.get_active_orders(
                account_index=adapter.account_index
            )
            raw = resp.to_dict() if hasattr(resp, 'to_dict') else {}
