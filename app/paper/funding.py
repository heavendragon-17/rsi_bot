# app/paper/funding.py
"""
PaperFundingScheduler
=====================
Applies real Binance USDT-M funding rates to open paper positions
...
class PaperFundingScheduler:
    """
    Background thread that applies Binance funding rates to open paper positions
    at each 8-hour funding window.
    """

    def __init__(self, state: "PaperTradeState", notifier: "PaperTelegramNotifier"):
        self._state = state
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run,
            name="PaperFundingScheduler",
            daemon=True,
        )
        self._thread.start()
        logger.info("[PaperFundingScheduler] Started")

    def stop(self) -> None:
        self._stop_event.set()
        if self._thread:
            self._thread.join(timeout=5)
        logger.info("[PaperFundingScheduler] Stopped")
    ...
    def _run(self) -> None:
        while not self._stop_event.is_set():
            ...
            logger.info(f"[PaperFundingScheduler] Next funding in {wait_secs:.0f}s")
    ...
    def apply_funding(self) -> None:
        """Fetch rates from Binance and deduct funding from open paper positions."""
        with self._state.lock:
            symbols = list(self._state.positions.keys())

        if not symbols:
            logger.debug("[PaperFundingScheduler] No open positions — skipping funding")
            return
        ...
        for symbol in symbols:
            try:
                ...
                logger.warning(
                    f"[PaperFundingScheduler] Funding rate fetch failed for {symbol}: {exc}. Skipping."
                )
                continue
            ...
            logger.info(
                f"[PaperFundingScheduler] {symbol} funding applied: "
                f"rate={float(rate):.6f}  payment={float(payment):.4f} USDT"
            )
            try:
                self._notifier.on_funding(symbol, rate, payment, balance_after)
            except Exception:
                logger.exception("[PaperFundingScheduler] notifier.on_funding error")
