...
def build_positions(
    ...
    ``direction="long"`` (default): BUY without exit reason opens, SELL exits.
    ``direction="short"`` (research-only mirror): SELL without exit reason
    opens, BUY exits, and realized P&L is sign-flipped.  The default path is
    byte-identical to the long-only behavior.
    """
    ...
    for row in sorted(trade_history, key=_fill_order_key):
        ...
        if side == entry_side and not reason:
        ...
        elif side == exit_side and open_pos is not None:
            ...
            if remaining <= dust_tol:
                ...
                positions.append(_finish_position(open_pos, risk_amount, direction))
...
def _finish_position(pos: dict[str, Any], risk_amount: D, direction: str = "long") -> dict[str, Any]:
    entry_qty = D(pos["entry_qty"])
    entry_fill = D(pos["entry_fill"])
    exits_value = sum((D(f["qty"]) * D(f["price"]) for f in pos["fills"]), D("0"))
    if direction == "short":
        gross = entry_qty * entry_fill - exits_value
    else:
        gross = exits_value - entry_qty * entry_fill
