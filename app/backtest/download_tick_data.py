...
def _download_zip(url: str, output_dir: str, label: str) -> str | None:
    ...
    if response.status_code != 200:
        print(f"  ✗ Failed (HTTP {response.status_code}) — {label}")
        return None
...
def download_days_range(symbol: str, start: date, end: date, output_dir: str) -> list[str]:
    ...
    print(f"  📅 Daily ticks: {start} → {end} ({total} days) ...")
    ...
    print(f"  ✓ Downloaded {len(paths)}/{total} days ({failed} failed/unavailable)")
    return paths
...
def merge_csvs(csv_paths: list[str], output_path: str) -> None:
    ...
    print(f"\nMerging {len(csv_paths)} file(s) → {output_path} ...")
    ...
    with open(output_path, 'w', newline='') as out_f:
        ...
        for path in sorted(csv_paths):
            with open(path, 'r', newline='') as in_f:
                ...
                for row in reader:
                    writer.writerow(row)
    print(f"✓ Merged file ready: {output_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--symbol", type=str, default="BTCUSDT",
                        help="Trading pair without slash (e.g. ZILUSDT)")
    parser.add_argument("--year",   type=int, default=None,
                        help="Year to download (e.g. 2026). Defaults to current year.")
    parser.add_argument("--month",  type=int, default=None,
                        help="Month to download (1-12). Defaults to last completed month.")
    parser.add_argument("--months", type=int, default=1,
                        help="Number of consecutive months to download (going backwards).")
    ...
    parser.add_argument("--merge",  action="store_true",
                        help="Merge all downloaded data into a single CSV file.")
    ...
    if args.recent:
        ...
        print(f"\n📊 Downloading last {args.recent} months of ticks for {clean_symbol}")
        print(f"   Coverage target: {target_start} → {now}")
        if monthly_plan:
            print(f"   Monthly archives: {len(monthly_plan)} months")
        if daily_range:
            print(f"   Daily archives:   {daily_range[0]} → {daily_range[1]}")
        print()
        ...
        daily_paths = []
        if daily_range:
            daily_paths = download_days_range(
                args.symbol, daily_range[0], daily_range[1], args.output
            )
        ...
        if not downloaded:
            ...
            for p in downloaded:
                os.remove(p)
            print(f"\n✓ Single output: {merged_path}")
        else:
            print(f"\n✓ Single file downloaded: {downloaded[0]}")
    ...
    # ── classic --year/--month/--months mode ───────────────────────────────
        ...
        print(f"\nDownloading {args.months} month(s) for {clean_symbol}:")
        ...
        if not downloaded:
            ...
            merged_name = (
                f"{clean_symbol}_ticks_{oldest_y}{oldest_m:02d}"
                f"_to_{newest_y}{newest_m:02d}.csv"
            )
            merged_path = os.path.join(args.output, merged_name)
            merge_csvs(downloaded, merged_path)
            ...
            for p in downloaded:
                os.remove(p)
            print(f"\nIndividual monthly files removed. Single output: {merged_path}")
        elif args.merge and len(downloaded) == 1:
            print("\nOnly one file downloaded — no merge needed.")
        else:
            print(f"\nDownloaded {len(downloaded)} file(s). Use --merge to combine into one.")
