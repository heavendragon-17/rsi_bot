...
def execute_order(
    ...
    fee_rate = fee_override if fee_override is not None else (
        exc.taker_fee if order_type.upper() in ("MARKET", "STOP_MARKET") else exc.maker_fee)
    ...
    order = {
        ...
        "margin": float(margin_used), "leverage": float(exc.leverage),
...
def check_liquidation(exc, timestamp: Any) -> bool:
    ...
    if equity <= Decimal("0"):
        ...
        for symbol in list(exc.positions.keys()):
            ...
            try:
                execute_order(exc, symbol, exit_side, abs_amt, price,
...
def update_stop_loss(
    ...
    sl_ids = [
        oid for oid, o in exc._sim.pending_orders.items()
...
def calc_hold_duration(entry_time: Any, exit_time: Any) -> Optional[float]:
