...
class MockExchange(IExchange):
    ...
    def pending_orders(self) -> Dict[str, Dict]:
        ...
        for oid, o in self._sim.pending_orders.items():
            result[oid] = {
                ...
                "type": o.order_type, "order_subtype": o.order_type,
                "status": "open", "reduce_only": o.reduce_only,
                "info": dict(o.info), "limit_price": o.limit_price,
                "callback_rate": o.callback_rate, "peak_price": o.peak_price,
            }
        return result
    ...
    def update_candle(
        self, symbol: str, open_: Any, high: Any, low: Any, close: Any, timestamp: Any,
    ) -> List[Dict]:
        with self._lock:
            ...
            for fr in fill_results:
                ...
                result = self._execute_order(
                    symbol=fr.symbol, side=fr.side, amount=fr.fill_amount,
                    exec_price=fr.fill_price, timestamp=timestamp,
                    order_type=fr.order_type.upper(), exit_reason=exit_reason,
                )
                if result:
    ...
    def fetch_positions(self, symbols: Optional[List[str]] = None) -> List[Dict]:
        with self._lock:
            ...
            for s, amt in self.positions.items():
                ...
                upnl = (curr - entry) * amt
                pos_list.append({
                    "symbol": s, "contracts": float(amt), "contractSize": 1.0,
                    "unrealizedPnl": float(upnl), "leverage": float(self.leverage),
    ...
    def create_order(
        self, symbol: str, order_type: str = None, side: str = None,
        amount: Any = None, price: Any = None, params: Optional[Dict] = None,
    ) -> Optional[Dict]:
        with self._lock:
            params = params or {}
