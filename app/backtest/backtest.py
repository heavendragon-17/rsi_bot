...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Trading symbol (e.g. XPL/USDT). If not provided, inferred from filename",
    )
    parser.add_argument(
        "--timeframe", type=str, default=None, help="Timeframe (e.g. 5m). If not provided, inferred from filename"
    )
    parser.add_argument(
        "--output", type=str, default=os.path.join(SCRIPT_DIR, "report"), help="Output directory for reports"
    )
    parser.add_argument(
        "--strategy",
        type=str,
    ...
    if strategy_name not in STRATEGY_MAP:
        logger.error("unknown_strategy", strategy=strategy_name, available=list(STRATEGY_MAP.keys()))
        sys.exit(1)
    ...
    # Initialize Engine
    engine = BacktestEngine(data_path=args.data, strategy_class=strategy_class, config=config)
    ...
    logger.info("saving_reports", output_dir=args.output)
    ...
    if report_path:
        logger.info("opening_report", path=report_path)
