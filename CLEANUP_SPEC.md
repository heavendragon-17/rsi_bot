...
*   **Remove** `place_stop_loss` / `place_take_profit` custom methods if they exist as public API (use `create_order` with `params={'stopLoss': ...}` or distinct `type='stop_loss'`). *Correction:* CCXT uses `create_order` with `params` for SL/TP or specific types. For this Mock, standardize on `create_order(..., params={'stopPrice': ...})`.
